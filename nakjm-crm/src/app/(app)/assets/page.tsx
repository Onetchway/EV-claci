...
const blankForm = {
  name: "", category: "TOOLS_MACHINERY" as AssetCategory, serialNumber: "", cost: "",
...
export default function AssetsPage() {
  ...
  async function create() {
    if (!form.name.trim() || !form.cost) throw new Error("Name and cost are required.");
    await createAsset({
      name: form.name,
  ...
  return (
    <>
      <PageHeader
        ...
        description="Equipment and machinery tracked from procurement through their depreciable life — an internal view, not a substitute for your statutory books."
        actions={
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="No assets yet"
          description="Equipment received against a purchase order, or added directly, shows up here with a running depreciation schedule."
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Name" required className="sm:col-span-2">
            <Input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="500 kVA transformer — Site #4" />
          </Field>
