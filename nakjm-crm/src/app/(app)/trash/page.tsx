...
import type { Actor, Asset, Project } from "@/lib/types";
import { formatDateTime } from "@/lib/utils";

type Trashable = { id: string; deletedAt?: unknown; deletedBy?: Actor | null };
      ...
      {selected.size > 0 && (
        <div className="flex items-center justify-between gap-3 rounded-lg bg-ink-900 px-4 py-2.5 text-sm text-white">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSelected(new Set())} className="text-ink-300 hover:text-white">Clear</button>
            <Button
              size="sm"
              variant="primary"
              loading={busy}
            ...
            {canForever && (
              <Button size="sm" variant="danger" onClick={() => onRequestPermaDelete(config.kind, rows.filter((r) => selected.has(r.id)))}>
      ...
      <div className="card overflow-x-auto scroll-thin">
        <table className="w-full">
          <thead className="border-b border-ink-200 bg-ink-50">
            <tr>
              <th className="th w-8">
                <input type="checkbox" checked={rows.length > 0 && rows.every((r) => selected.has(r.id))} onChange={toggleAll} aria-label={`Select all trashed ${config.pluralLabel.toLowerCase()}`} />
              </th>
          ...
          <tbody className="divide-y divide-ink-100">
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-ink-50">
                <td className="td w-8">
                  <input type="checkbox" checked={selected.has(row.id)} onChange={() => toggle(row.id)} aria-label={`Select ${config.primary(row)}`} />
                </td>
                <td className="td">
                ...
                <td className="td text-ink-500">{formatDateTime(row.deletedAt as never)}</td>
                <td className="td">
                  <div className="flex items-center gap-1.5">
                    <Button size="sm" variant="primary" loading={busy} onClick={() => void run(() => config.restore(row, actor), `${config.label} restored.`)}>
...
export default function TrashPage() {
  ...
  useEffect(() => {
    ...
    const pending = new Set(["projects", "assets"]);
  ...
  const totalCount = projects.length + assets.length;
  ...
  const permaLabel = useMemo(() => {
    ...
    return sections.find((s) => s.kind === permaTarget.kind)?.label.toLowerCase() ?? "item";
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [permaTarget]);

  async function permanentlyDelete(kind: Kind, rows: Trashable[]) {
    if (kind === "project") { for (const r of rows) await deleteProject(r as unknown as Project); }
  ...
  return (
    <>
      <PageHeader
        title="Trash"
        description="Deleted projects and assets stay here, fully recoverable, until permanently deleted."
      />
      ...
      <Modal
        ...
        description="This cannot be undone — the record and anything linked to it are gone for good."
        footer={
          <>
