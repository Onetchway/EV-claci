...
import { createQuotation } from "@/lib/db/quotations";
...
function NewQuotationInner() {
  ...
  if (!canManageQuotations(viewer)) {
    return <p className="text-sm text-ink-500">You don't have permission to create quotations.</p>;
  }
  ...
  return (
    <>
      <PageHeader
        title="New quotation"
        description={leadCode ? `Prefilled from lead ${leadCode} — client details can still be edited below.` : "For a client buying chargers and/or an EPC service — doesn't need to be an existing lead."}
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          <Card title="Terms">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Valid until" hint="Optional — shown on the printed quotation.">
                <Input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} />
              </Field>
            </div>
            <Field label="Notes" className="mt-4">
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Internal notes, or terms to show on the quotation." />
            </Field>
          </Card>
        </div>
