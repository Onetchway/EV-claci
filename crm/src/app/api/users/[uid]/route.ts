...
export async function PATCH(req: Request, { params }: { params: { uid: string } }) {
  try {
    ...
    for (const key of ["name", "phone", "region", "managerId", "designation", "department", "active", "orgId", "pageAccessOverrides", "bypassGeofence", "hrmsAdmin", "attendanceRequired"] as const) {
