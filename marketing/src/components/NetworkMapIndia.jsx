...
import { MapPin, Zap, Battery, Map as MapIcon, Building2, Wifi, Radio } from 'lucide-react';
...
const PINS = [
  { x: 97.0, y: 106.0, label: 'Delhi NCR', side: 'left' },
  { x: 132.9, y: 122.3, label: 'Lucknow', side: 'right' },
  { x: 105.7, y: 90.1, label: 'Dehradun', side: 'right' },
];
    ...
    <section className="bg-white py-24">
      <div className="container-lv grid gap-14 lg:grid-cols-[280px_1fr_280px] lg:items-center">
        ...
        <ScrollReveal delay={0.1} className="relative mx-auto w-full max-w-sm">
          <div aria-hidden="true" className="absolute inset-0 flex items-center justify-center">
            {[1, 0.72, 0.44].map((s) => (
              <span
                ...
                style={{ width: `${s * 130}%`, height: `${s * 130}%` }}
              />
            ))}
          </div>
          <div
          ...
          />
          <svg viewBox="0 0 300 340" className="relative w-full">
            <defs>
          ...
          {/* Pill labels, absolutely positioned over the map */}
          {PINS.map((p, i) => (
            <motion.div
              key={p.label}
              ...
              className={
                'absolute flex -translate-y-1/2 items-center gap-1.5 whitespace-nowrap rounded-full bg-white px-3 py-1.5 text-xs font-bold text-ink shadow-md ' +
                (p.side === 'left' ? '-translate-x-full' : '')
              }
              style={{
                left: `${(p.x / 300) * 100}%`,
                top: `${(p.y / 340) * 100}%`,
                marginLeft: p.side === 'left' ? -10 : 10,
              }}
            >
              {p.label}
            </motion.div>
        ...
        <ScrollReveal delay={0.15}>
          ...
          <div className="relative mt-8 overflow-hidden rounded-2xl border border-line bg-surface-alt p-5">
            <div className="absolute right-0 top-0 h-14 w-14 overflow-hidden">
              <div className="absolute right-[-28px] top-[10px] flex h-8 w-20 rotate-45 items-center justify-center bg-brand-500 text-white shadow">
            ...
            <div className="mt-3 font-display text-sm font-bold uppercase tracking-wide">Lucknow</div>
            <dl className="mt-3 space-y-2 text-xs">
              <div className="flex items-center justify-between"><dt className="flex items-center gap-1.5 text-muted"><MapPin className="h-3 w-3" />Region</dt><dd className="font-semibold">Uttar Pradesh</dd></div>
              <div className="flex items-center justify-between"><dt className="flex items-center gap-1.5 text-muted"><Zap className="h-3 w-3" />Charger type</dt><dd className="font-semibold">AC & DC</dd></div>
              <div className="flex items-center justify-between"><dt className="flex items-center gap-1.5 text-muted"><Wifi className="h-3 w-3" />Status</dt><dd className="font-semibold text-brand-600">Live</dd></div>
            </dl>
          </div>

          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-surface-alt px-3 py-1 text-muted">
              <Radio className="h-3 w-3 text-brand-500" />Live locations
            </span>
          </div>
        </ScrollReveal>
      </div>
