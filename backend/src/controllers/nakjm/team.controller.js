...
exports.listByProject       = async (req, res, next) => { try { res.json(await svc.listByProject(req.params.projectId)); } catch (e) { next(e); } };
exports.assignToProject     = async (req, res, next) => { try { res.status(201).json(await svc.assignToProject(req.params.projectId, req.body)); } catch (e) { next(e); } };
exports.unassignFromProject = async (req, res, next) => { try { await svc.unassignFromProject(req.params.projectId, req.params.teamMemberId); res.status(204).end(); } catch (e) { next(e); } };
