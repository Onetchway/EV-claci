...
const create = async (data) => {
  ...
  try {
    ...
    const res = await client.query(
      `INSERT INTO nakjm_quotations (id, quotation_no, project_id, client_id, version, status, quotation_date, valid_until, subtotal, tax_percent, tax_amount, total_amount, terms, notes, source_boq_id, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7,CURRENT_DATE),$8,$9,$10,$11,$12,$13,$14,$15,NOW(),NOW()) RETURNING *`,
      [id, quotation_no, project_id, client_id, version, status, quotation_date, valid_until, subtotal, tax_percent, taxAmount, total, terms, notes, source_boq_id]
    );
...
const update = async (id, data) => {
  ...
  if (fields.length) {
    ...
    params.push(id);
    const res = await query(`UPDATE nakjm_quotations SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id`, params);
    if (!res.rows[0]) { const e = new Error('Quotation not found'); e.status = 404; throw e; }
  }
  return getOne(id);
};
...
const remove = async (id) => {
  const res = await query('DELETE FROM nakjm_quotations WHERE id = $1 RETURNING id', [id]);
  if (!res.rows[0]) { const e = new Error('Quotation not found'); e.status = 404; throw e; }
};
