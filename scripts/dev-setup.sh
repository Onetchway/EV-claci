...
write_env() {
  local file="$1"; shift
  # Always overwritten, not just written when missing — this script is the
  # source of truth for these four dev .env files, and leaving a stale one
  # in place (from before a fix to this script, or a different PGUSER/
  # PGPASSWORD/port) is exactly what caused several confusing failures
  # here: a wrong password baked in from an earlier run, then a wrong port,
  # neither ever getting corrected on a "PGUSER=... ./dev-setup.sh" re-run.
  printf '%s\n' "$@" > "$file"
  echo "    wrote $file"
}
